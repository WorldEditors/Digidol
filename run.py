...
async def main():
    ...
    available_roles = [d for d in os.listdir("roles")
                       if os.path.isdir(os.path.join("roles", d))]
    roles_hint = "/".join(available_roles)
    role_name = input(f"请输入角色名称 ({roles_hint}): ").strip() or available_roles[0]

    model_config = config_mgr.get_active_model()
    active_model_name = model_config.get("name", "未知") if model_config else "未配置"
    print(f"\n当前模型: {active_model_name}")

    if not model_config or not model_config.get("api_key"):
        print("⚠️ 请先在 Web 界面配置 API Key: streamlit run config/web_config.py")
        return
    ...
    agent = VLMAgent(character, config=model_config)
    ...
    while True:
        ...
        if user_input.lower() == "voice":
            test_text = f"你好呀！我是{character.profile.get('display_name', role_name)}，很高兴认识你！"
        ...
        print(f"{character.profile.get('display_name', role_name)}: {response['text']}")
